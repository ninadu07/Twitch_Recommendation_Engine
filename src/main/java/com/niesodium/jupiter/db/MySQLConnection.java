package com.niesodium.jupiter.db;

import com.niesodium.jupiter.entity.Item;
import com.niesodium.jupiter.entity.ItemType;
import com.niesodium.jupiter.entity.User;
...
public class MySQLConnection {
    ...
    //1. avoid wasting DB resources
    //2. -> connect -> read -> return, read以后不需要connection， 本身程序运行当中可以释放无效数据/资源
    public void close() {
        if (conn != null) {
            try {
                conn.close();
    ...
    public void setFavoriteItem(String userId, Item item) throws MySQLException {
        ...
        saveItem(item);
        //IGNORE: 如果插入数据有duplicates， 就ignore， 不会抛出异常， 不然log里会频繁出现exception
        //用sql本身的方法instead of String.format, 防止恶意攻击和删库跑路：）
    ...
    public boolean addUser(User user) throws MySQLException {
        ...
        try {
            ...
            return statement.executeUpdate() == 1; //row count
        } catch (SQLException e) {
            e.printStackTrace();
